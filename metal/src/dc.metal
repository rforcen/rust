...
struct DomainColorInput {
    color device*image;
    uint  device&side;
};

kernel void DomainColoring( DomainColorInput device&dc [[buffer(0)]], 
                          uint2 position [[thread_position_in_grid]] ) // 0..w*h
{
    dc.image[ pos2index(position, dc.side) ] = pixelColorzFunc(position.x, position.y, dc.side, dc.side);
}
