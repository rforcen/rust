...
pub fn freq_in_octave(_freq : f64, oct : i32) -> f64 {
	...
	if freq!=0.0 {		
		...
		if freq>fb { while freq!=0. && freq > fb && !(freq>=fb && freq<=ft) { freq/=2. } }  // in octave
		else    { while freq!=0. && freq < ft && !(freq>=fb && freq<=ft) { freq*=2. } }
	}
	freq
}

pub fn freq2oct_note(_freq : f64) -> (i8, u8) {
	let lfb = if _freq != 0.0 { _freq.log(E) - LOG_BASEC0 } else {0.0};
