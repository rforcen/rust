...
// zvm evaluator
float2 eval_zvn(float2 z, global uint *code) {
  enum Symbols {
    SNULL = 0,
    NUMBER = 1,
    IDENTi = 2,
...
#define MAX_STACK 128
  ...
  for (;;) {
    switch (code[pc]) {
    case PUSHC:
      stack[sp++] = (float2)(*(global float *)(code + (++pc)), 0);
      break;
    case PUSHZ:
      stack[sp++] = z;
      break;
...
uint dc_get_color(int x, int y, int w, int h, global uint *code) {
