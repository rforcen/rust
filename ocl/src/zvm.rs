...
const FUNC_NAMES: [&'static str; 15] = [
    "sin", "cos", "tan", "exp", "log", "log10", "int", "sqrt", "asin", "acos", "atan", "abs", "c",
    "pi", "phi",
];
...
impl ZVm {
    ...
    fn u32_to_f32(u: u32) -> f32 {
        f32::from_ne_bytes(u.to_ne_bytes())
    }
    fn f32_to_u32(f: f32) -> u32 {
        u32::from_ne_bytes(f.to_ne_bytes())
    }
    fn u32_2_sym(c: u32) -> Symbols {
        unsafe { ::std::mem::transmute(c as u8) }
    }
    fn usize_2_sym(c: usize) -> Symbols {
        unsafe { ::std::mem::transmute(c as u8) }
    }
    ...
    fn getsym(&mut self) -> Symbols {
        ...
        if self.ch.is_alphabetic() {
            ...
            if self.ident == "z" {
                self.sym = Symbols::IDENTz
            } else if self.ident == "i" {
                self.sym = Symbols::IDENTi
            } else {
                // func ?
                ...
                if index.is_some() {
                    // sym = FSIN + index
                    self.sym =
                        unsafe { ::std::mem::transmute(Symbols::FSIN as u8 + index.unwrap() as u8) }
                } else {
                    // error
                    self.sym = Symbols::SNULL
                }
    ...
    pub fn eval(&self, z: CF32) -> CF32 {
        ...
        loop {
            match Self::u32_2_sym(self.code[pc]) {
                ...
                Symbols::FC => {
                    sp -= 1;
                    stack[sp - 1] = CF32::new(stack[sp - 1].re, stack[sp].re)
                }
